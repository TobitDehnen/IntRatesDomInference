...
get.real.ints <- function(n_males = 16, # Number of males
                          ...
                          ratio_ints_to_MM_MF_FF_dyads = c(9,6,3), # Number of times each MM, MF, FF dyad interacts
                          male_aggro_bias = "Males interact with non-breeding females only", # Whether males don't interact aggresively with subordinate females (TRUE) or if interact same as with other females (FALSE)
  ...
  # Individuals and their dominance_real_metric
  inds <- data.frame(ID = LETTERS[1:(n_males+n_females)], # IDs
                     dominance_real_metric = sort(c(rnorm((n_males), mean = 0, sd = 1), # distribution higher for biomdal (males and females)
                                        rnorm((n_females), mean = -4, sd = 1)), # distribution lower for bimodal (males and females)
                                      decreasing = TRUE), # dominance metric
                     dominance_real_order = 1:(n_males + n_females), # dominance order
                     sex = c(rep("M", times = n_males), rep("F", times = n_females))) # sex
  
  # Standardise dominance_real_metric between 0 and no. individuals in group -1
  inds$dominance_real_metric <- (inds$dominance_real_metric - min(inds$dominance_real_metric)) / (max(inds$dominance_real_metric) - min(inds$dominance_real_metric)) * ((n_males+n_females) - 1)
  # Add also the order for later use
  inds$dominance_real_order <- 1:nrow(inds)
  ...
  ints_real_MM <- ints_real_MM[rep(seq_len(nrow(ints_real_MM)), times = ratio_ints_to_MM_MF_FF_dyads[1]),] # repeat certain number of times
  # MF
  ints_real_MF <- expand.grid(Ind_A = inds$ID[which(inds$sex == "M")], Ind_B = inds$ID[which(inds$sex == "F")]) # get all combinations
  ints_real_MF <- ints_real_MF[rep(seq_len(nrow(ints_real_MF)), times = ratio_ints_to_MM_MF_FF_dyads[2]),] # repeat certain number of times
  # FF
  ...
  ints_real_FF <- ints_real_FF[rep(seq_len(nrow(ints_real_FF)), times = ratio_ints_to_MM_MF_FF_dyads[3]),] # repeat certain number of times
  ...
  for (i in 1:nrow(ints_real)) {
    ...
    # Get rank difference
    rank_diff <- inds$dominance_real_metric[which(inds$ID == Ind_A_tmp)] - inds$dominance_real_metric[which(inds$ID == Ind_B_tmp)]
  ...
  if(male_aggro_bias == "Males interact with non-breeding females only") {
...
get.Elo <- function(ints_obs # dataframe holding interactions, containing: ID, winner and loser columns
  ...
  # Create output file
  dominance_real_metric_df <- data.frame(ID = names(mean_scores), metric = unname(mean_scores))[order(mean_scores, decreasing = T),]
  dominance_real_metric_df$order <- 1:nrow(dominance_real_metric_df)
  dominance_real_metric_df$method <- "Elo_rand"
  
  return(dominance_real_metric_df)
}
